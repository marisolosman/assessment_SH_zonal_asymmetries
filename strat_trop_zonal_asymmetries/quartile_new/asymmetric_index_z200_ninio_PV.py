...
for i in np.arange(0, 7):
	...
	np.savez(PATH_DATA_2 + 'z200_conditioned_' + month[i] + '_new.npz', var1=hgt.z.values[i, index_ninio_all.values, :, :], var2=hgt.z.values[i, :, :, :], var3=hgt.z.values[i, index_ninia_all.values, :, :], var4=hgt.z.values[i, index_ninio_WPV, :, :], var5=hgt.z.values[i, index_ninia_WPV, :, :], var6=hgt.z.values[i, index_ninio_SPV, :, :], var7=hgt.z.values[i, index_ninia_SPV, :, :])
...
ds.to_netcdf(PATH_DATA_2 + 'monthly_correlations_enso_SPoV_polar_new.nc4')
...
for i in np.arange(0, 5):
	...
	np.savez(PATH_DATA_2 + 'z200_conditioned_' + seas[i] + '_new.npz', var1=hgt_s.z.values[index_ninio_all.values, :, :], var2=hgt_s.z.values[:, :, :], var3=hgt_s.z.values[index_ninia_all.values, :, :], var4=hgt_s.z.values[index_ninio_WPV, :, :], var5=hgt_s.z.values[index_ninia_WPV, :, :], var6=hgt_s.z.values[index_ninio_SPV, :, :], var7=hgt_s.z.values[index_ninia_SPV, :, :])
	#test correlation
	correl_ninio_WPV[i] = TestCorrelation(var_ninio_all, var_normal, var_ninio_WPV, var_normal)
	correl_ninia_WPV[i] = TestCorrelation(var_ninia_all, var_normal, var_ninia_WPV, var_normal)
	correl_ninio_SPV[i] = TestCorrelation(var_ninio_all, var_normal, var_ninio_SPV, var_normal)
...
ds.to_netcdf(PATH_DATA_2 + 'seasonal_correlations_enso_SPoV_polar_new.nc4')
