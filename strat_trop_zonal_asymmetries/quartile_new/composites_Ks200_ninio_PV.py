...
Rt = np.transpose(np.tile(6.378e6 * np.cos(winds.latitude.values * np.pi / 180),
			  [nmonth, nrealiz, nlons, 1]), [0, 1, 3, 2])
Umean = winds.mean(dim='longitude')
Umean = np.transpose(np.tile(Umean.values, [nlons, 1, 1, 1]), [1, 2, 3, 0])
...
for i in np.arange(7):
	...
	var_normal = np.nanmean(Ks[i, :, :, :], axis=0)
	...
	plots.PlotKsEnsoCompositesPoV(var_ninio_all, var_ninia_all,
				    var_ninio_WPV, var_ninia_WPV, # - var_normal,
				    var_ninio_SPV, var_ninia_SPV, # - var_normal,
				    winds.latitude, winds.longitude, tit, filename)
for i in np.arange(5):
	winds_s = winds.isel(month=range(i, i+3)).mean(dim='month')
	Umean = np.transpose(np.tile(winds_s.mean(dim='longitude'), [nlons, 1, 1]),
			     [1, 2, 0])
	...
	var_normal = np.nanmean(Ks[:, :, :], axis=0)
	...
	plots.PlotKsEnsoCompositesPoV(var_ninio_all, var_ninia_all,
				    var_ninio_WPV, var_ninia_WPV,
				    var_ninio_SPV, var_ninia_SPV,
				    winds.latitude, winds.longitude, tit, filename)
