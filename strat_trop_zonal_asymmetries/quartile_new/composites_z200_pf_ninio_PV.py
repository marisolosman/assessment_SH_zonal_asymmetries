...
#composites of plum fluxes and its divergence for EN conditioned on PV strength
import numpy as np
import xarray as xr
import os
import plots
...
for i in np.arange(0, 7):
	...
	px_ninio_WPV, py_ninio_WPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninio_WPV-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	...
	px_ninia_WPV, py_ninia_WPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninia_WPV-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	#div_ninia_WPV = calc.divergence(px_ninia_WPV, py_ninia_WPV, dx, dy)

	var_ninio_SPV = np.mean(hgt.z.values[i, index_ninio_SPV, :, :], axis=0)
	px_ninio_SPV, py_ninio_SPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninio_SPV-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	#div_ninio_SPV = calc.divergence(px_ninio_SPV, py_ninio_SPV, dx, dy)

	var_ninia_SPV = np.mean(hgt.z.values[i, index_ninia_SPV, :, :], axis=0)	
	px_ninia_SPV, py_ninia_SPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninia_SPV-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	#div_ninia_SPV = calc.divergence(px_ninia_SPV, py_ninia_SPV, dx, dy)

	var_ninio_all = np.mean(hgt.z.values[i, index_ninio_all, :, :], axis=0)
	px_ninio_all, py_ninio_all, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninio_all-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	#div_ninio_all = calc.divergence(px_ninio_all, py_ninio_all, dx, dy)

	var_ninia_all = np.mean(hgt.z.values[i, index_ninia_all, :, :], axis=0)	
	px_ninia_all, py_ninia_all, lat = plumb_flux.ComputePlumbFluxes(winds_clm.u.values[i, :, :], winds_clm.v.values[i, :, :], var_ninia_all-var_normal, np.zeros_like(var_ninio_WPV), hgt.latitude.values, hgt.longitude.values)
	...
	filename = FIG_PATH + 'z200_pf_composites_ENSO_' + month[i] +'_SPoV_q.png'
...
#	tit = 'Composites S4 Plumb Fluxes and divergence 200hPa Conditioned - SPoV - ' + month[i]
#	filename = FIG_PATH + 'div_pf_composites_ENSO_' + month[i] +'_SPoV_q.png'
...
for i in np.arange(0, 5):
	...
	filename = FIG_PATH + 'z200_pf_composites_ENSO_' + seas[i] +'_SPoV_q.png'
	var ={'z1': var_ninio_all-var_normal, 'px1': px_ninio_all, 'py1': py_ninio_all,
	      'z2': var_ninia_all-var_normal, 'px2': px_ninia_all, 'py2': py_ninia_all,
	      'z3': var_ninio_WPV-var_normal, 'px3': px_ninio_WPV, 'py3': py_ninio_WPV,
	      'z4': var_ninia_WPV-var_normal, 'px4': px_ninia_WPV, 'py4': py_ninia_WPV,
...
#	      'z5': div_ninio_SPV, 'px5': px_ninio_SPV, 'py5': py_ninio_SPV,
#	      'z6': div_ninia_SPV, 'px6': px_ninia_SPV, 'py6': py_ninia_SPV}
#	tit = 'Composites S4 Plumb Fluxes and divergencePF (*1e7) 200hPa Conditioned - SPoV - ' + month[i]
#	filename = FIG_PATH + 'div_pf_composites_ENSO_' + seas[i] +'_SPoV_q.png'
#	plots.PlotEnsoCompositesPoVdivPF(var, lat, hgt.longitude, tit, filename)
#
