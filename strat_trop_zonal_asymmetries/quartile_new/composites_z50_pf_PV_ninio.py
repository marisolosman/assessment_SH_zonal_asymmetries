...
for i in np.arange(0, 5):
	...
	px_WPV, py_WPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_WPV_all - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	px_SPV, py_SPV, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_SPV_all - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	px_WPV_EN, py_WPV_EN, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_WPV_EN - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	px_SPV_EN, py_SPV_EN, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_SPV_EN - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	px_WPV_LN, py_WPV_LN, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_WPV_LN - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	px_SPV_LN, py_SPV_LN, lat = plumb_flux.ComputePlumbFluxes(winds_clm_s.u.values, winds_clm_s.v.values, var_SPV_LN - var_normal, np.zeros_like(var_WPV_EN), hgt.latitude.values, hgt.longitude.values, 5000)
	var ={'z1': var_SPV_all - var_normal, 'px1': px_SPV, 'py1': py_SPV,
	      'z2': var_SPV_EN - var_normal, 'px2': px_SPV_EN, 'py2': py_SPV_EN,
	      'z3': var_SPV_LN - var_normal, 'px3': px_SPV_LN, 'py3': py_SPV_LN,
	      'z4': var_WPV_all - var_normal, 'px4': px_WPV, 'py4': py_WPV,
	      'z5': var_WPV_EN - var_normal, 'px5': px_WPV_EN, 'py5': py_WPV_EN,
	      'z6': var_WPV_LN - var_normal, 'px6': px_WPV_LN, 'py6': py_WPV_LN}
	filename = FIG_PATH + 'z50_pf_composites_SPoV_' + seas[i] +'_ENSO_q.png'
	tit = 'Composites S4 Z* and Plum Fluxes 50hPa SPoV Conditioned - ENSO - ' + seas[i]
	plots.PlotPoVCompositesENSOZPF(var, hgt.latitude, hgt.longitude, tit, filename)
