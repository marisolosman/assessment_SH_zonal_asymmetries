...
for i in np.arange(0, 7):
	var_normal = np.mean(winds.u.values[i, :, :, :], axis=0)
	var_ninio_WPV = np.mean(winds.u.values[i, index_ninio_WPV, :, :], axis=0)
	var_ninia_WPV = np.mean(winds.u.values[i, index_ninia_WPV, :, :], axis=0)	
	var_ninio_SPV = np.mean(winds.u.values[i, index_ninio_SPV, :, :], axis=0)
	var_ninia_SPV = np.mean(winds.u.values[i, index_ninia_SPV, :, :], axis=0)	
	var_ninio_all = np.mean(winds.u.values[i, index_ninio_all, :, :], axis=0)
	var_ninia_all = np.mean(winds.u.values[i, index_ninia_all, :, :], axis=0)	
	...
	tit = 'Composites S4 U 200hPa Conditioned - SPoV - ' + month[i]
	filename = FIG_PATH + 'U200_composites_ENSO_' + month[i] +'_SPoV_q.png'
...
for i in np.arange(0, 5):
	...
	var_ninio_WPV = np.mean(hgt_s.u.values[index_ninio_WPV, :, :], axis=0)
	var_normal = np.mean(hgt_s.u.values[:, :, :], axis=0)
	var_ninia_WPV = np.mean(hgt_s.u.values[index_ninia_WPV, :, :], axis=0)
	var_ninio_SPV = np.mean(hgt_s.u.values[index_ninio_SPV, :, :], axis=0)
	var_ninia_SPV = np.mean(hgt_s.u.values[index_ninia_SPV, :, :], axis=0)
	var_ninio_all = np.mean(hgt_s.u.values[index_ninio_all, :, :], axis=0)
	var_ninia_all = np.mean(hgt_s.u.values[index_ninia_all, :, :], axis=0)
	tit = 'Composites S4 U 200hPa Conditioned - SPoV - ' + seas[i]
	filename = FIG_PATH + 'U200_composites_ENSO_' + seas[i] +'_SPoV_q.png'
	var ={'z1': var_ninio_all-var_normal,# 'px1': px_ninio_all, 'py1': py_ninio_all,
	      'z2': var_ninia_all-var_normal,# 'px2': px_ninia_all, 'py2': py_ninia_all,
	      'z3': var_ninio_WPV-var_normal,# 'px3': px_ninio_WPV, 'py3': py_ninio_WPV,
	      'z4': var_ninia_WPV-var_normal,# 'px4': px_ninia_WPV, 'py4': py_ninia_WPV,
