...
for i in np.arange(7):
	...
	tit = 'Composites S4 RWS and Divergent wind 200hPa - ' + month[i] + ' SPoV'
	filename = FIG_PATH + '200_RWS_chi_composites_PV_' + month[i] +'_ENSO.png'
...
for i in np.arange(5):
	...
	uchi_all = np.mean(uchi.u_chi[i, :, :, :], axis=2)
	vchi_all = np.mean(vchi.v_chi[i, :, :, :], axis=2)
	tit = 'Composites S4 RWS and Divergent wind 200hPa - ' + seas[i] + ' SPoV'
	filename = FIG_PATH + '200_RWS_chi_composites_PV_' + seas[i] +'_ENSO.png'
	plots.PlotCompositesRWSChiWPVENSO(var_SPV_all - var_all,
					       var_SPV_EN - var_all,
					       var_SPV_LN - var_all,
					       var_WPV_all - var_all,
