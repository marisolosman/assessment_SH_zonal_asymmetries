...
for i in np.arange(0, 7):
	...
	np.savez(PATH_DATA_2 + 'z50_PV_conditioned_ENSO_' + month[i] + '.npz', var1=hgt.z.values[i, index_SPV_all.values, :, :], var2=hgt.z.values[i, index_WPV_all.values, :, :], var3=hgt.z.values[i, :, :, :], var4=hgt.z.values[i, index_SPV_EN, :, :], var5=hgt.z.values[i, index_WPV_EN, :, :], var6=hgt.z.values[i, index_SPV_LN, :, :], var7=hgt.z.values[i, index_WPV_LN, :, :])
	#test correlation
	correl_SPV_EN[i] = TestCorrelation(var_SPV_all, var_normal, var_SPV_EN, var_normal)
	correl_WPV_EN[i] = TestCorrelation(var_WPV_all, var_normal, var_WPV_EN, var_normal)
	correl_SPV_LN[i] = TestCorrelation(var_SPV_all, var_normal, var_SPV_LN, var_normal)
	correl_WPV_LN[i] = TestCorrelation(var_WPV_all, var_normal, var_WPV_LN, var_normal)
	correl_SPV[i] = TestCorrelation(var_SPV_EN, var_normal, var_SPV_LN, var_normal)
	correl_WPV[i] = TestCorrelation(var_WPV_EN, var_normal, var_WPV_LN, var_normal)
...
ds.to_netcdf(PATH_DATA_2 + 'monthly_correlations_z50_SPoV_enso_polar.nc4')
...
for i in np.arange(0, 5):
	...
	np.savez(PATH_DATA_2 + 'z50_PV_conditioned_ENSO_' + seas[i] + '.npz',  var1=hgt_s.z.values[index_SPV_all.values, :, :], var2=hgt_s.z.values[index_WPV_all.values, :, :], var3=hgt_s.z.values[:, :, :],  var4=hgt_s.z.values[index_SPV_EN, :, :], var5=hgt_s.z.values[index_WPV_EN, :, :], var6=hgt_s.z.values[index_SPV_LN, :, :], var7=hgt_s.z.values[index_WPV_LN, :, :])
...
ds.to_netcdf(PATH_DATA_2 + 'seasonal_correlations_z50_SPoV_enso_polar.nc4')
