...
for i in np.arange(0, 7):
	...
	var_WPV_EN = np.mean(harm1[i, index_WPV_EN, :, :], axis=0)
	var_WPV_LN = np.mean(harm1[i, index_WPV_LN, :, :], axis=0)	
	var_SPV_EN = np.mean(harm1[i, index_SPV_EN, :, :], axis=0)	
	var_SPV_LN = np.mean(harm1[i, index_SPV_LN, :, :], axis=0)
	var_WPV_all = np.mean(harm1[i, index_WPV_all, :, :], axis=0)	
	var_SPV_all = np.mean(harm1[i, index_SPV_all, :, :], axis=0)	
	tit = 'Composites S4 Z* 50hPa Wave-1 Conditioned - ENSO - ' + month[i]
	filename = FIG_PATH + 'z50W1_composites_SPoV2_' + month[i] +'_ENSO.png'
	...
	var_WPV_EN = np.mean(harm2[i, index_WPV_EN, :, :], axis=0)
	var_WPV_LN = np.mean(harm2[i, index_WPV_LN, :, :], axis=0)	
	var_SPV_EN = np.mean(harm2[i, index_SPV_EN, :, :], axis=0)	
	var_SPV_LN = np.mean(harm2[i, index_SPV_LN, :, :], axis=0)
	var_WPV_all = np.mean(harm2[i, index_WPV_all, :, :], axis=0)	
	var_SPV_all = np.mean(harm2[i, index_SPV_all, :, :], axis=0)	
	tit = 'Composites S4 Z* 50hPa Wave-2 Conditioned - ENSO - ' + month[i]
	filename = FIG_PATH + 'z50W2_composites_SPoV2_' + month[i] +'_ENSO.png'
	plots.PlotPoVCompositesENSO(var_SPV_all - var_normal, var_WPV_all - var_normal,
				    var_SPV_EN - var_normal, var_WPV_EN - var_normal,
				    var_SPV_LN - var_normal, var_WPV_LN - var_normal,
				    var_normal, harms.latitude, harms.longitude, tit, filename)
...
for i in np.arange(0, 5):
	...
	var_WPV_all = np.mean(harm1[i, index_WPV_all, :, :], axis=0)	
	var_SPV_all = np.mean(harm1[i, index_SPV_all, :, :], axis=0)	
	tit = 'Composites S4 Z* 50hPa Wave-1 Conditioned - ENSO - ' + seas[i]
	filename = FIG_PATH + 'z50W1_composites_SPoV2_' + seas[i] +'_ENSO.png'
	plots.PlotPoVCompositesENSO(var_SPV_all - var_normal, var_WPV_all - var_normal,
				    var_SPV_EN - var_normal, var_WPV_EN - var_normal,
				    var_SPV_LN - var_normal, var_WPV_LN - var_normal,
				    var_normal, harms.latitude, harms.longitude, tit, filename)
	...
	var_WPV_all = np.mean(harm2[i, index_WPV_all, :, :], axis=0)	
	var_SPV_all = np.mean(harm2[i, index_SPV_all, :, :], axis=0)	
	tit = 'Composites S4 Z* 50hPa Wave-2 Conditioned - ENSO - ' + seas[i]
	filename = FIG_PATH + 'z50W2_composites_SPoV2_' + seas[i] +'_ENSO.png'
	plots.PlotPoVCompositesENSO(var_SPV_all - var_normal, var_WPV_all - var_normal,
				    var_SPV_EN - var_normal, var_WPV_EN - var_normal,
				    var_SPV_LN - var_normal, var_WPV_LN - var_normal,
				    var_normal, harms.latitude, harms.longitude, tit, filename)
