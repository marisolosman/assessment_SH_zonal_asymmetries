...
for i in np.arange(7):
	...
	tit = 'Composites S4 RWS and Divergent wind 200hPa - ' + month[i] + ' ENSO'
	filename = FIG_PATH + '200_RWS_chi_composites_ENSO_' + month[i] +'_SPoV.png'
...
S = xr.open_dataset(RUTA + 'seasonal_RWS.nc4', chunks={'latitude':10})
...
for i in np.arange(5):
	...
	uchi_all = np.mean(uchi.u_chi[i, :, :, :], axis=2)
	vchi_all = np.mean(vchi.v_chi[i, :, :, :], axis=2)
	tit = 'Composites S4 RWS and Divergent wind 200hPa - ' + seas[i] + ' ENSO'
	filename = FIG_PATH + '200_RWS_chi_composites_ENSO_' + seas[i] +'_SPoV.png'
	plots.PlotCompositesRWSChiWENSOPV(var_EN_all - var_all,
					  var_EN_WPV - var_all,
					  var_EN_SPV - var_all,
					  var_LN_all - var_all,
