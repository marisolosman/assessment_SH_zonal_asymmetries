...
#lista = xr.open_mfdataset(RUTA + "correlations/seasonal_correlations_z200_enso_SPoV_*_q.nc4",
#			  parallel=True, combine='nested', chunks=10000, concat_dim='iter')

#lista.to_netcdf(RUTA + 'seasonal_correlations_z200_enso_SPoV_10k_q.nc4')
lista = xr.open_dataset(RUTA + 'seasonal_correlations_z200_enso_SPoV_10k_q.nc4')
file = RUTA + "seasonal_correlations_enso_SPoV_polar_q.nc4"
...
df.to_csv(RUTA + "percentile_seasonal_correlations_composites_z200_enso_SPoV_q.csv")


lista = xr.open_mfdataset(RUTA + "correlations/monthly_correlations_z200_enso_SPoV_*_q.nc4",
			  parallel=True, combine='nested', chunks=10000, concat_dim='iter')

lista.to_netcdf(RUTA + 'monthly_correlations_z200_enso_SPoV_10k_q.nc4')

file = RUTA + "monthly_correlations_enso_SPoV_polar_q.nc4"
correlations = xr.open_dataset(file)
...
for ii in lista.data_vars:
	...
	df = df.append(pd.DataFrame({'Field': ii, 'Correlation':corr_coef, 'Percentile':pvalue,
					     ...
					     index=months, columns=['Field', 'Correlation',
							            'Percentile', 
					     '5th Percentile', '95th Percentile']))
df.to_csv(RUTA + "percentile_monthly_correlations_composites_z200_enso_SPoV_q.csv")
