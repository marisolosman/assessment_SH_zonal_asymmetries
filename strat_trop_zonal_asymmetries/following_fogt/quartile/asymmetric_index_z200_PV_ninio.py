...
for i in np.arange(0, 7):
	...
	np.savez(PATH_DATA_2 + 'z200_PV_conditioned_ENSO_' + month[i] + '_q.npz', var1=hgt.z.values[i, index_SPV_all.values, :, :], var2=hgt.z.values[i, index_WPV_all.values, :, :], var3=hgt_EN.z.values[i, index_SPV_EN.values, :, :], var4=hgt_EN.z.values[i, index_WPV_EN.values, :, :], var5=hgt_LN.z.values[i, index_SPV_LN.values, :, :], var6=hgt_LN.z.values[i, index_WPV_LN.values, :, :])
...
ds.to_netcdf(PATH_DATA_2 + 'monthly_asymmetric_index_SPoV_enso_polar_q.nc4')
ds = xr.Dataset({'correl_EN': (['month'], correl_EN),
		 'correl_LN': (['month'], correl_LN)},
		 coords={'month': (['month'], month)})
ds.to_netcdf(PATH_DATA_2 + 'monthly_correlations_SPoV_enso_polar_q.nc4')
...
for i in np.arange(0, 5):
	...
	np.savez(PATH_DATA_2 + 'z200_PV_conditioned_ENSO_' + seas[i] + '_q.npz',  var1=hgt_s.z.values[index_SPV_all.values, :, :], var2=hgt_s.z.values[index_WPV_all.values, :, :], var3=hgt_s_EN.z.values[ index_SPV_EN.values, :, :], var4=hgt_s_EN.z.values[index_WPV_EN.values, :, :], var5=hgt_s_LN.z.values[index_SPV_LN.values, :, :], var6=hgt_s_LN.z.values[index_WPV_LN.values, :, :])
...
ds.to_netcdf(PATH_DATA_2 + 'seasonal_asymmetric_index_SPoV_enso_polar_q.nc4')

ds = xr.Dataset({'correl_EN': (['seas'], correl_EN),
		 'correl_LN': (['seas'], correl_LN)},
		 coords={'seas': (['seas'], seas)})
ds.to_netcdf(PATH_DATA_2 + 'seasonal_correlations_SPoV_enso_polar_q.nc4')
