...
for i in np.arange(0, 7):
	...
	tit = 'Composites S4 Z* 50hPa Conditioned - WPV - ' + month[i]
	filename = './new_figures_decile/z50_composites_NINIO_' + month[i] +'_WPV.png'
...
for i in np.arange(0, 5):
	...
	tit = 'Composites S4 Z* 50hPa Conditioned - WPV - ' + seas[i]
	filename = './new_figures_decile/z50_composites_NINIO_' + seas[i] +'_WPV.png'
	plots.PlotEnsoComposites(var_pos, var_neg, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0, 7):
	...
	tit = 'Composites S4 Z* 50hPa Conditioned - SPV - ' + month[i]
	filename = './new_figures_decile/z50_composites_NINIO_' + month[i] +'_SPV.png'
	plots.PlotEnsoComposites(var_pos, var_neg, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0, 5):
	...
	var_pos = np.mean(var.z[index_monthly_upper.values, :, :], axis=0) - np.mean(var.z[index_monthly_normal.values, :, :], axis=0)
	var_neg = np.mean(var.z[index_monthly_lower.values, :, :], axis=0) - np.mean(var.z[index_monthly_normal.values, :, :], axis=0)
	tit = 'Composites S4 Z* 50hPa Conditioned - SPV - ' + seas[i]
	filename = './new_figures_decile/z50_composites_NINIO_' + seas[i] +'_SPV.png'
	plots.PlotEnsoComposites(var_pos, var_neg, ds.latitude, ds.longitude, tit, filename)
