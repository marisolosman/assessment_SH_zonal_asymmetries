...
for i in np.arange(0,7):
	...
	filename = './new_figures_decile/z200_composites_NINIO_' + month[i] +'.png'
	...
	tit = 'Composites Z* 200hPa diff EN-LN Years - ' + month[i]
	filename = './new_figures_decile/z200_composites_diff_NINIO_' + month[i] +'.png'
...
for i in np.arange(0, 5):
	...
	filename = './new_figures_decile/z200_composites_NINIO_' + seas[i] +'.png'
	plots.PlotEnsoComposites(var_pos, var_neg, ds.latitude, ds.longitude, tit, filename)
	var = np.mean(var.z[index_monthly_upper.values, :, :], axis=0) - np.mean(var.z[index_monthly_lower.values, :, :], axis=0)
	tit = 'Composites Z* 200hPa diff EN-LN Years - ' + seas[i]
	filename = './new_figures_decile/z200_composites_diff_NINIO_' + seas[i] +'.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
