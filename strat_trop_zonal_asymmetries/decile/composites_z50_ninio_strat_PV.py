...
for i in np.arange(0,7):
	...
	tit = 'HGT 50hPa Composites differences EN+LN Years - ' + month[i] + '- Weak PV'
	filename = './figures_decile/z50_composites_sum_NINIO_' + month[i] +'_WPV.png'
...
for i in np.arange(0,5):
	...
	tit = 'HGT 50hPa Composites differences EN+LN Years - ' + seas[i] + '- Weak PV'
	filename = './figures_decile/z50_composites_sum_NINIO_' + seas[i] +'_WPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0,7):
	var = np.mean(ds_PV.z.values[i, ~index_monthly_normal.values, :, :], axis=0) - np.mean(ds_PV.z.values[i, index_monthly_normal.values, :, :], axis=0)
	tit = 'HGT 50hPa Composites differences EN+LN Years - ' + month[i] + '- Strong PV'
	filename = './figures_decile/z50_composites_sum_NINIO_' + month[i] +'_SPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)

for i in np.arange(0,5):
	var = ds_PV.isel(month=range(i, i + 3)).mean(dim='month')
	var = np.mean(var.z[~index_monthly_normal.values, :, :], axis=0) - np.mean(var.z[index_monthly_normal.values, :, :], axis=0)
	tit = 'HGT 50hPa Composites differences EN-LN Years - ' + seas[i] + '- Strong PV'
	filename = './figures_decile/z50_composites_sum_NINIO_' + seas[i] +'_SPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
