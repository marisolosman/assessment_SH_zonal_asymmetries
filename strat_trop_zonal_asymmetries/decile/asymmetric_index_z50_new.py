...
for i in np.arange(0, 7):
	...
	np.savez(PATH_DATA_2 + 'z50_conditioned_' + month[i] + '_d_new.npz', var1=hgt.z.values[i, index_ninio_all.values, :, :], var2=hgt.z.values[i, index_normal_all.values, :, :], var3=hgt.z.values[i, index_ninia_all.values, :, :], var4=hgt.z.values[i, index_ninio_WPV, :, :], var5=hgt.z.values[i, index_normal_WPV, :, :], var6=hgt.z.values[i, index_ninia_WPV, :, :], var7=hgt.z.values[i, index_ninio_SPV, :, :], var8=hgt.z.values[i, index_normal_SPV, :, :], var9=hgt.z.values[i, index_ninia_SPV, :, :])
...
#testear si los campos son distintos:
	...
	correl_ninio[i] = TestCorrelation(var_ninio_WPV, var_normal_WPV, var_ninio_SPV, var_normal_SPV)
	correl_ninia[i] = TestCorrelation(var_ninia_WPV, var_normal_WPV, var_ninia_SPV, var_normal_SPV)
...
ds.to_netcdf(PATH_DATA_2 + 'monthly_correlations_z50_enso_SPoV_polar_d_new.nc4')
...
for i in np.arange(0, 5):
	...
	np.savez(PATH_DATA_2 + 'z50_conditioned_' + seas[i] + '_d_new.npz', var1=hgt_s.z.values[index_ninio_all.values, :, :], var2=hgt_s.z.values[index_normal_all.values, :, :], var3=hgt_s.z.values[index_ninia_all.values, :, :], var4=hgt_s.z.values[index_ninio_WPV, :, :], var5=hgt_s.z.values[index_normal_WPV, :, :], var6=hgt_s.z.values[index_ninia_WPV, :, :], var7=hgt_s.z.values[index_ninio_SPV, :, :], var8=hgt_s.z.values[index_normal_SPV, :, :], var9=hgt_s.z.values[index_ninia_SPV, :, :])
	#test correlation
	correl_ninio_WPV[i] = TestCorrelation(var_ninio_all, var_normal_all, var_ninio_WPV, var_normal_WPV)
	correl_ninia_WPV[i] = TestCorrelation(var_ninia_all, var_normal_all, var_ninia_WPV, var_normal_WPV)
	correl_ninio_SPV[i] = TestCorrelation(var_ninio_all, var_normal_all, var_ninio_SPV, var_normal_SPV)
...
ds.to_netcdf(PATH_DATA_2 + 'seasonal_correlations_z50_enso_SPoV_polar_d_new.nc4')
