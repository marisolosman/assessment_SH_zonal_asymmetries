...
ninio4_erai_seasonal = xr.concat([ninio4_aso_erai.sel(time=(ninio4_aso_erai['time.month']== 8)), ninio4_son_erai.sel(time=(ninio4_son_erai['time.month']==9))], dim='time').dropna(dim='time')

ninio4_erai_monthly = ninio4_erai.sel(time = np.logical_and(ninio4_erai['time.month']>=8, ninio4_erai['time.month']<=11))

[lamb, v, PC] = eofdata.eofdata(np.transpose(np.reshape(ninio4_erai_monthly.sst.values, [36, 4])), 4)
print(v[:, 0])
sst_monthly_index = -PC[0, :]

[lamb, v, PC] = eofdata.eofdata(np.reshape(ninio4_erai_seasonal.sst.values[~np.isnan(ninio4_erai_seasonal.sst.values)],[2, 36]), 8)
sst_seasonal_index = PC[0, :]
print(v[:, 0])
