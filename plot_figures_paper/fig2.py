...
def PlotMeanTest(t, F, lat_observ, lon_observ, title, filename):
	...
	cbar_ax = fig.add_axes([0.27, 0.1, 0.5, 0.05])
...
if not(os.path.isfile( ROUTE + 'datos_fig2.npz')):
	...
	for i in np.arange(0, 7):
		...
		F[i, :, :] = ma.masked_array(F[i, :, :], mask=np.logical_and(F[i, :, :]>df_cut[0],
					     F[i, :, :]<df_cut[1]))
	title = 'GPH 50hPa Model Bias (m)'
	filename = FIG_PATH + 'fig3.eps'
	PlotMeanTest(tt, F, lat_erai, lon_erai, title, filename)
	title = 'GPH 50hPa Variance ratio'
	filename = FIG_PATH + 'fig4.eps'
	PlotStdTest(tt, F, lat_erai, lon_erai, title, filename)
	np.savez('datos_fig2.npz', tt=tt, F=F, lat_erai=lat_erai, lon_erai=lon_erai)
