...
FILE_HGT_S4 = 'hgt200_monthly_harmonics.nc4'
FILE_HGT_CLIM_S4 = 'hgt200_monthly_clim_harmonics.nc4'
FILE_NINIO_S4 = 'fogt/ninio34_monthly.nc4'
FILE_PV_S4 = 'fogt/SPV_index.nc4'
hgt = xr.open_dataset(PATH_DATA_2 + FILE_HGT_S4)
hgt_clim = xr.open_dataset(PATH_DATA_2 + FILE_HGT_CLIM_S4)
...
var_ninio_WPV = np.mean(hgt.Harmonics.values[:, :, index_ninio_WPV, :, :], axis=2)
SS_ninio_WPV = np.var(hgt.Harmonics.values[:, :, index_ninio_WPV, :, :], axis=2) / nn_ninio_WPV
var_ninia_WPV = np.mean(hgt.Harmonics.values[:, :, index_ninia_WPV, :, :], axis=2)	
SS_ninia_WPV = np.var(hgt.Harmonics.values[:, :, index_ninia_WPV, :, :], axis=2) / nn_ninia_WPV
var_ninio_SPV = np.mean(hgt.Harmonics.values[:, :, index_ninio_SPV, :, :], axis=2)
SS_ninio_SPV = np.var(hgt.Harmonics.values[:, :, index_ninio_SPV, :, :], axis=2) / nn_ninio_SPV
var_ninia_SPV = np.mean(hgt.Harmonics.values[:, :, index_ninia_SPV, :, :], axis=2)	
SS_ninia_SPV = np.var(hgt.Harmonics.values[:, :, index_ninia_SPV, :, :], axis=2) / nn_ninia_SPV
var_ninio_all = np.mean(hgt.Harmonics.values[:, :, index_ninio_all.values, :, :], axis=2)
SS_ninio_all = np.var(hgt.Harmonics.values[:, :, index_ninio_all.values, :, :], axis=2) / nn_ninio_all
var_normal = np.mean(hgt.Harmonics.values[:, :, :, :, :], axis=2)
SS_normal = np.var(hgt.Harmonics.values[:, :, :, :, :], axis=2) / nn_index_normal
var_SPV_all = np.mean(hgt.Harmonics.values[:, :, index_SPV_lower.values, :, :], axis=2)
SS_SPV_all = np.var(hgt.Harmonics.values[:, :, index_SPV_lower.values, :, :], axis=2) / nn_SPV_lower
var_WPV_all = np.mean(hgt.Harmonics.values[:, :, index_SPV_upper.values, :, :], axis=2)
SS_WPV_all = np.var(hgt.Harmonics.values[:, :, index_SPV_upper.values, :, :], axis=2) / nn_SPV_upper
var_ninia_all = np.mean(hgt.Harmonics.values[:, :, index_ninia_all.values, :, :], axis=2)
SS_ninia_all = np.var(hgt.Harmonics.values[:, :, index_ninia_all.values, :, :], axis=2) / nn_ninia_all

tit = 'Composites S4 Z* 200hPa Wave-1'
filename = FIG_PATH + 'z200_W1_composites_ENSO_SPoV2.eps'
...
for i in range(4):
	...
	ax[0, i].coastlines()
	variable = var_WPV_all[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_WPV_all[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	...
	im = ax[0, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
...
#
	...
	ax[1, i].coastlines()
	variable = var_SPV_all[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_SPV_all[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_SPV_lower + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[1, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[1, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			linewidths=0.4, colors='black')
	barra.set_under(barra(0))
	barra.set_over(barra(barra.N-1))
	...
	ax[2, i].coastlines()
	variable = var_ninio_all[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninio_all[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_ninio_all + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[2, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[2, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			 linewidths=0.4, colors='black')
	...
	ax[3, i].coastlines()
	variable = var_ninia_all[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninia_all[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_ninia_all + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[3, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[3, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			 linewidths=0.4, colors='black')
	...
	ax[4, i].coastlines()
	variable = var_ninio_WPV[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninio_WPV[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_ninio_WPV + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[4, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[4, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			 linewidths=0.4, colors='black')
	...
	ax[5, i].coastlines()
	variable = var_ninia_SPV[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninia_SPV[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_ninia_SPV + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[5, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[5, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			 linewidths=0.4, colors='black')
	...
	ax[6, i].coastlines()
	variable = var_ninio_SPV[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninio_SPV[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	cota = stats.t.ppf(1 - 0.025, nn_ninio_SPV + nn_index_normal - 2)
	M = np.ma.array(variable, mask= np.abs(variable_std) < cota)
	im = ax[6, i].contourf(lon, lat, M, clevs, transform=ccrs.PlateCarree(),
			 cmap=barra, extend='both', vmin=-60, vmax=60)
	ax[6, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			linewidths=0.4, colors='black')
	...
	ax[7, i].coastlines()
	variable = var_ninia_WPV[harmonic + 1, i + 1, :, :] - var_normal[harmonic + 1, i + 1, :, :]
	variable_std = variable / np.sqrt(SS_ninia_WPV[harmonic + 1, i + 1, :, :] + SS_normal[harmonic + 1, i + 1, :, :])
	...
	barra.set_under(barra(0))
	barra.set_over(barra(barra.N-1))
	ax[7, i].contour(lon, lat, hgt_clim.Harmonics[harmonic + 1, i +1, :, :], clevs2, transform=ccrs.PlateCarree(),
			linewidths=0.4, colors='black')
pad = 5
