...
for i in np.arange(0,4):
    ...
    var = np.mean(ds.z[i, index_Plo_Sup, :, :], axis=0) -\
	  np.mean(ds.z[i, index_Pup_Sup, :, :], axis=0)
...
#    
    ...
    var = np.mean(ds.z[i, index_Plo_Slo, :, :], axis=0) -\
	  np.mean(ds.z[i, index_Pup_Slo, :, :], axis=0)
    CS1 = mapproj.contourf(lonproj, latproj, var, clevs, cmap=barra, extend='both') 
    barra.set_under(barra(0))
    barra.set_over(barra(barra.N-1))
    plt.title('Composites S-PV - W-PV cond -SOI YEARS')
    ...
    plt.savefig('hgt_200_composites_cond_SOI_PV_' + month[i] +'.png', dpi=300,
...
for i in np.arange(0,2):
    ...
    barra = plt.cm.RdBu_r #colorbar
    if i == 0:
        var = np.mean(ds.z.sel(**{'month':slice(8,10)}).mean(dim='month')[index_Plo_Sup, :, :], axis=0)
        var = var - np.mean(ds.z.sel(**{'month':slice(8,10)}).mean(dim='month')[index_Pup_Sup, :, :], axis=0)
    else:
        var = np.mean(ds.z.sel(**{'month':slice(9,11)}).mean(dim='month')[index_Plo_Sup, :, :], axis=0)
        var = var - np.mean(ds.z.sel(**{'month':slice(9, 11)}).mean(dim='month')[index_Pup_Sup, :, :], axis=0)
    CS1 = mapproj.contourf(lonproj, latproj, var, clevs, cmap=barra, extend='both') 
    barra.set_under(barra(0))
    barra.set_over(barra(barra.N-1))
    plt.title('Composites S-PV - W-PV cond +SOI YEARS')
...
#    
    ...
    barra = plt.cm.RdBu_r #colorbar
    if i == 0:
        var = np.mean(ds.z.sel(**{'month':slice(8,10)}).mean(dim='month')[index_Plo_Slo, :, :], axis=0) - np.mean(ds.z.sel(**{'month':slice(8,10)}).mean(dim='month')[index_Pup_Slo, :, :], axis=0)
    else:
        var = np.mean(ds.z.sel(**{'month':slice(9,11)}).mean(dim='month')[index_Plo_Slo, :, :], axis=0) - np.mean(ds.z.sel(**{'month':slice(9,11)}).mean(dim='month')[index_Pup_Slo, :, :], axis=0)
#
    CS1 = mapproj.contourf(lonproj, latproj, var, clevs, cmap=barra, extend='both') 
    barra.set_under(barra(0))
    barra.set_over(barra(barra.N-1))
    ...
    fig.colorbar(CS1, cax=cbar_ax, orientation='horizontal')
    #cbar_ax.set_xticklabels([0, 40, 80, 120, 160], size=9)
    plt.savefig('hgt_200_composites_cond_SOI_PV_' + seas[i] +'.png', dpi=300,
		bbox_inches='tight', orientation='landscape',
		papertype='A4')
