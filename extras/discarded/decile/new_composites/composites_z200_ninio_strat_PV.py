...
for i in np.arange(0,7):
	...
	tit = 'S4 Z* 200hPa Neutral ENSO years - ' + month[i] + '- Weak PV'
	filename = './new_figures_decile/z200_neutral_NINIO_' + month[i] +'_WPV.png'
...
for i in np.arange(0,5):
	...
	tit = 'S4 Z* 200hPa Neutral ENSO years - ' + seas[i] + '- Weak PV'
	filename = './new_figures_decile/z200_neutral_NINIO_' + seas[i] +'_WPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0,7):
	var = np.mean(ds_PV.z.values[i, index_monthly_normal.values, :, :], axis=0)
	tit = 'S4 Z* 200hPa Neutral ENSO years - ' + month[i] + '- Strong PV'
	filename = './new_figures_decile/z200_neutral_NINIO_' + month[i] +'_SPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)

for i in np.arange(0,5):
	var = ds_PV.isel(month=range(i, i + 3)).mean(dim='month')
	var = np.mean(var.z[index_monthly_normal.values, :, :], axis=0)
	tit = 'S4 Z* 200hPa Neutral ENSO years - ' + seas[i] + '- Strong PV'
	filename = './new_figures_decile/z200_neutral_NINIO_' + seas[i] +'_SPV.png'
	plots.PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
