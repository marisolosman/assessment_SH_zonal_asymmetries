...
for i in np.arange(0, 7):
	...
	filename = './figures/hgt_200_composites_diff_PV_' + month[i] +'_EN.png'
...
for i in np.arange(0, 5):
	...
	filename = './figures/hgt_200_composites_diff_PV_' + seas[i] +'_EN.png'
	PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0, 7):
	...
	filename = './figures/hgt_200_composites_diff_PV_' + month[i] +'_LN.png'
	PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
...
for i in np.arange(0, 5):
	...
	var = np.mean(var.z[index_monthly_lower.values, :, :], axis=0) - np.mean(var.z[index_monthly_upper.values, :, :], axis=0)
	tit = 'Composites differences SPV-WPV Years - ' + seas[i]
	filename = './figures/hgt_200_composites_diff_PV_' + seas[i] +'_LN.png'
	PlotCompDiff(var, ds.latitude, ds.longitude, tit, filename)
