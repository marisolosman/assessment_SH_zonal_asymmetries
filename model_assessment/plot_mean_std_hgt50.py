...
def PlotMeanStd(model_m, model_sd, lat_model, lon_model, observ_m, observ_sd,
	...
	ax.contour(lon_observ, lat_observ, observ_m,
		   levels=np.arange(17000, 21000, 500), linewidths=0.4,
...
for i in np.arange(0, 7):
	title = 'HGT 50-hPa Mean (contour) and SD (shaded) - ' + month[i]
	filename = './figures/hgt50_mu_sd_' + month[i] + '.png'
...
for i in np.arange(0, 5):
	...
	title = 'HGT 50-hPa Mean (contour) and SD (shaded) - ' + season[i]
	filename = './figures/hgt50_mu_sd_' + season[i] + '.png'
	PlotMeanStd(np.mean(hgt_s4_smean, axis=0), np.std(hgt_s4_smean, axis=0), lat_s4,
		    lon_s4, np.mean(hgt_erai_smean, axis=0), np.std(hgt_erai_smean, axis=0),
