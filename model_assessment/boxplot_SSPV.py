...
ROUTE = '~/datos/data/' #estes spanglish no te lo robo amiga
...
index_sninio_erai_upper = ninio34_erai.ninio34_mon >= ninio34_erai.ninio34_mon.quantile(0.90, dim='dim_0')
index_sninio_erai_lower = ninio34_erai.ninio34_mon <= ninio34_erai.ninio34_mon.quantile(0.10, dim='dim_0')
index_sninio_erai_normal = np.logical_and(ninio34_erai.ninio34_mon < ninio34_erai.ninio34_mon.quantile(0.90, dim='dim_0'), ninio34_erai.ninio34_mon > ninio34_erai.ninio34_mon.quantile(0.10, dim='dim_0'))
...
index_sninio_s4_upper = ninio34_s4.ninio34_mon >= ninio34_s4.ninio34_mon.quantile(0.90, dim='dim_0')
index_sninio_s4_lower = ninio34_s4.ninio34_mon <= ninio34_s4.ninio34_mon.quantile(0.10, dim='dim_0')
index_sninio_s4_normal = np.logical_and(ninio34_s4.ninio34_mon < ninio34_s4.ninio34_mon.quantile(0.90, dim='dim_0'), ninio34_s4.ninio34_mon > ninio34_s4.ninio34_mon.quantile(0.10, dim='dim_0'))
...
# add x-tick labels
plt.savefig('./figures/boxplot_SAM.png', res=400)
...
for i in range(5):
	results = sm.OLS(SAM_erai[i, :], sm.add_constant(ninio34_erai.ninio34_mon.values)).fit()
	...
	plt.subplot(3, 2, i + 1)
	plt.scatter(ninio34_erai.ninio34_mon.values, SAM_erai[i,:])
	...
	plt.title(season[i] + '- correlation: ' + '{:03.2f}'.format(np.corrcoef(ninio34_erai.ninio34_mon.values, SAM_erai[i, :]) [0,1]))
plt.suptitle('Regression SAM vs ENSO - ERAI')
plt.subplots_adjust(wspace=0.3, hspace=0.30)
plt.savefig('./figures/regression_sam_enso.png', res=400, box_inches='tight'
)
limite = np.max(np.abs(ninio34_s4.ninio34_mon.values))
#scatter plot between ENSO and SAM
fig = plt.figure(figsize=(14, 12), dpi=300)
for i in range(5):
	results = sm.OLS(SAM_s4[i, :], sm.add_constant(ninio34_s4.ninio34_mon.values)).fit()
	...
	interv = results.conf_int(alpha=0.05)
	plt.subplot(3, 2, i + 1)
	plt.scatter(ninio34_s4.ninio34_mon.values, SAM_s4[i,:])
	plt.plot(x_pred, y_pred)
	plt.fill_between(x_pred, lower, upper, color='#888888', alpha=0.1)
	...
	plt.xlabel('Ninio 3.4')
	plt.ylabel('Zonal SAM index')
	plt.title(season[i] + '- correlation: ' + '{:03.2f}'.format(np.corrcoef(ninio34_s4.ninio34_mon.values, SAM_s4[i, :])[0,1]))
plt.subplots_adjust(wspace=0.3, hspace=0.30)
plt.suptitle('Regression SAM vs ENSO - S4')
plt.savefig('./figures/regression_sam_enso_s4.png', res=400,bbox_inches='tight' )
