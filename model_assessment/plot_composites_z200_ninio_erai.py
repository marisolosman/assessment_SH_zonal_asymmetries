...
for i in [1]: #np.arange(0, 5):
	...
	hgt_s4_smean = np.nanmean(np.reshape(hgt_s4_smean,
					     [36, 51, lon_s4.shape[0], lat_s4.shape[1]]), 1)	
	hgt_s4_EN = np.nanmean(hgt_s4_smean[index_ninio_erai_upper.values, :, :], axis=0)
	hgt_s4_LN = np.nanmean(hgt_s4_smean[index_ninio_erai_lower.values, :, :], axis=0)
	hgt_s4_N = np.nanmean(hgt_s4_smean[index_ninio_erai_normal.values, :, :], axis=0)
	#composites EN-N
	title = 'Composites Z* 200hPa NINIO years - ' + season[i]
	filename = FIG_PATH + 'Composites_NINIO_' + season[i] + '_2.png'
